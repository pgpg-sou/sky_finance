function changeCalcYears(e) {
    ...
    e ? t < terms.length - 1 && (selectedTerm = terms[t + 1], performCalculations()) : t > 0 && (selectedTerm = terms[t - 1], performCalculations()), gaSend("Calculator", "Term", "Term", selectedTerm.name, "Calculator"), $(".calc-term-holder").text(selectedTerm.name)
}

function refreshCalcs(e) {
    var t = accounting.formatMoney(e, "", 0, ",");
    $(".form-control-borrow-mini").val(t), sls.length > 1 ? $.each(sls, function(t, a) {
        a.bootstrapSlider("setValue", e)
...
function alterCalcAmount(e) {
    var t = parseCalcAmount($(".form-control-borrow-mini").val()),
        a = 7500;
    e ? (t >= CALC_INCREMENT_THRESHOLD ? (a = t + 500, a > CALC_MAX_VALUE && (a = CALC_MAX_VALUE)) : (a = t + CALC_INCREMENT, a > CALC_MAX_VALUE && (a = CALC_MAX_VALUE)), gaSend("Calculator", "Increase amount", "Increase amount", a, "Calculator")) : (t >= CALC_INCREMENT_THRESHOLD ? (a = t - 500, CALC_MIN_VALUE > a && (a = CALC_MIN_VALUE)) : (a = t - CALC_INCREMENT, CALC_MIN_VALUE > a && (a = CALC_MIN_VALUE)), gaSend("Calculator", "Decrease amount", "Decrease amount", a, "Calculator")), refreshCalcs(a)
}

function parseCalcAmount(e) {
    var t = e.replace(/\D+/g, ""),
        a = parseInt(t, 10);
    return isNaN(a) ? a = 0 : a > CALC_MAX_VALUE ? a = CALC_MAX_VALUE : 0 > a && (a = 0), a
}
...
function CreditScore(e, t, a, s) {
    this.name = e, this.bestApr = t, this.repApr = a, this.activated = s
}

function Input(e, t, a) {
    this.amount = e, this.term = t, this.creditScore = a
}

function Output(e) {
    this.input = e, this.calculate = function(e) {
        var t = accounting.unformat(this.input.amount.toString()),
            a = accounting.unformat(this.input.term.value.toString()),
            s = accounting.unformat("0"),
            r = 0,
            o = 0;
        r = accounting.unformat(e ? this.input.creditScore.repApr.toString() : this.input.creditScore.bestApr.toString());
        var n = Math.pow(1 + r, 1 / 12),
            c = Math.pow(n, a),
            l = Math.pow(n, a + 1 - 1),
            i = (t + o) * (1 - n) * (l / (1 - c)),
            u = i * a,
            d = u - (t - s);
        this.totalRepayment = accounting.formatMoney(u, "", 2, ","), this.totalCost = accounting.formatMoney(d, "", 2, ","), this.monthlyRepayments = accounting.formatMoney(i, "", 2, ",")
    }
}

function performCalculations() {
    var e, t = new Input;
    t.amount = parseCalcAmount($(".form-control-borrow-mini").val()), t.term = selectedTerm, t.creditScore = CreditScoreEnum.getCreditScore($(".user-input-creditscore").val()), (isNaN(t.amount) || t.amount <= 0) && (t.amount = 0), e = new Output(t), e.calculate(), saveOptionsInCookies(t), refreshResults(e)
}

function calculateExample() {
    var e, t = new Input;
    t.amount = CALC_DEFAULT_AMOUNT, t.term = TermOptionsEnum.getTermFromValue(CALC_DEFAULT_TERM), t.creditScore = CreditScoreEnum.getCreditScore($(".user-input-creditscore").val()), e = new Output(t), e.calculate(!0), refreshExample(e)
}
...
function saveOptionsInCookies(e) {
    var t = {
        ...
        a = "LoanAmount=" + t.LoanAmount + "&LoanTerm=" + t.LoanTerm + "&Apr=" + t.Apr;
}
...
function refreshExample(e) {
    $(".rep-example-credit-name").text(e.input.creditScore.name), $(".rep-example-amount").text(e.input.amount), $(".rep-example-term").text(e.input.term.name), $(".rep-example-rate").text(formatCalcPercentage(e.input.creditScore.repApr)), $(".rep-example-monthly").text("£" + e.monthlyRepayments), $(".rep-example-cost-credit").text("£" + e.totalCost), $(".rep-example-total-repayment").text("£" + e.totalRepayment)
}
...
function getAllElementsWithAttribute(e) {
    for (var t = [], a = document.getElementsByTagName("*"), s = 0, r = a.length; r > s; s++) null !== a[s].getAttribute(e) && t.push(a[s]);
    return t
}

function gaSend(e, t, a, s, r) {
    dataLayer.push({
        event: r,
        eventCategory: e,
        eventValue: s,
...
function ExecuteSearch(e) {
    e = e.replace("/", " "), $("#searchAreaInput").val(e);
    ...
    "" !== e ? ($(".numberOfResults").text(""), $.ajax({
        ...
        success: function(a) {
            if ($(".faq-search-results").removeClass("hidden"), $(".faq-results-found__count").removeClass("hidden"), $(".searchTerm").text(e), a.length >= 1) {
                t = 1, $(".faq-search-results__copyright-row").removeClass("hidden").hide().fadeIn(500), $(".faq-result-found").removeClass("hidden"), $(".faq-not-found").addClass("hidden"), $(".childquestions").addClass("hidden"), $(".faq-search-results").addClass("hidden"), $(".numberOfResults").text(t), $(".faq-search-results__topic").text(a[0].questionTitle);
                var s = a[0].answerText.substr(0, 285) + "...",
                    r = document.createElement("a");
                a[0].linkText = a[0].linkText.replace("%2f", "-"), r.setAttribute("href", a[0].linkText), r.innerHTML = s;
                $(".faq-search-results__copyright-text").html(r)
        ...
        complete: function() {
            $.ajax({
                ...
                success: function(s) {
                    var r = [];
                    t += s.length, $(".faq-results-found__count").removeClass("hidden"), $(".faq-search-results").removeClass("hidden"), s.length >= 1 ? (s.length <= 5 ? ($(".faq-view-more").addClass("hidden"), $("#reveal-more-results-btn").addClass("hidden")) : ($(".faq-view-more").removeClass("hidden"), $("#reveal-more-results-btn").removeClass("hidden"), $(".rev-more").removeClass("hidden")), $(".faq-result-found").removeClass("hidden"), $(".faq-not-found").addClass("hidden"), $(".childquestions").removeClass("hidden"), $(".faq-results-list").removeClass("hidden"), $(".numberOfResults").text(t), $(".searchTerm").text(e), $.each(s, function(e, t) {
                        {
                            var a = t.questionTitle,
                                s = t.question;
                            t.questionText
                        }
                        t.linkText = t.linkText.replace("%2f", "-");
                        var o = $("<li></li>").append($("<a></a>").attr("href", t.linkText).append($('<div class="questionSearched"></div>').html(a)).append($('<div class="answerSearched"></div>').html(s)));
                        r.push(o)
                    }), $('ul[class="faq-results-list"]').append.apply($('ul[class="faq-results-list"]'), r).hide().fadeIn(500), $("html, body").animate({
                        scrollTop: $(".faq-results-found__count").offset().top
                    }, 750), $(".childquestions").html(), $(".faq-results-list").empty(), $(".faq-results-list").append.apply($(".faq-results-list"), r), highlight(a)) : 1 > t && ($(".faq-not-found").removeClass("hidden"), $(".faq-results-found__count").addClass("hidden"), $(".childquestions").addClass("hidden"), $('ul[class="faq-results-list"]').addClass("hidden"), $(".rev-more").addClass("hidden"))
                }
            })
        }
...
$(document).ready(function() {
    var e = [CreditScoreEnum.Excellent, CreditScoreEnum.Good, CreditScoreEnum.Fair, CreditScoreEnum.Bad],
        t = !1;
    terms = [TermOptionsEnum.Term1, TermOptionsEnum.Term2, TermOptionsEnum.Term3, TermOptionsEnum.Term4, TermOptionsEnum.Term5], selectedTerm = terms[3], selectCredits = $(".user-input-creditscore"), selectCredits.on("change", function(e) {
        selectCredits.val(e.target.value), performCalculations(), calculateExample(), gaSend("Calculator", "select credit", "select credit", e.target.value, "Calculator")
    }), $.each(selectCredits, function(t, a) {
        $.each(e, function(e, t) {
            $(a).append(new Option(t.name, t.name))
        })
    }), sls = $(".slider").bootstrapSlider({
        min: CALC_MIN_VALUE,
        max: CALC_MAX_VALUE,
        step: CALC_INCREMENT,
...
var TermOptionsEnum = new function() {
        this.Term1 = new Term("1 year", 12), this.Term2 = new Term("2 years", 24), this.Term3 = new Term("3 years", 36), this.Term4 = new Term("4 years", 48), this.Term5 = new Term("5 years", 60), this.getTermFromValue = function(e) {
            var t = parseInt(e);
            return t === this.Term1.value ? this.Term1 : t === this.Term2.value ? this.Term2 : t === this.Term3.value ? this.Term3 : t === this.Term4.value ? this.Term4 : t === this.Term5.value ? this.Term5 : this.Term1
        }
    },
    CreditScoreEnum = new function() {
        this.Excellent = new CreditScore("Excellent", .07, .105, "active"), this.Good = new CreditScore("Good", .092, .132, ""), this.Fair = new CreditScore("Fair", .132, .183, ""), this.Bad = new CreditScore("Bad", .191, .27, ""), this.getCreditScore = function(e) {
            switch (e) {
                case "Excellent":
...
$(document).ready(function() {
        ...
        $(getAllElementsWithAttribute("data-ga-item")).click(function(e) {
            var t = $(e.delegateTarget).data("ga-label"),
                a = $(e.delegateTarget).data("ga-action"),
                s = $(e.delegateTarget).data("ga-category"),
                r = $(e.delegateTarget).data("ga-event"),
                o = $(e.delegateTarget).data("ga-value");
            gaSend(s, a, t, o, r)
        })
    }),
    function(e, t, a) {
        "undefined" != typeof module && module.exports ? module.exports = a() : "function" == typeof define && define.amd ? define(a) : t[e] = a()
    }("jquery-scrollto", this, function() {
        var e, t, a;
        return e = t = window.jQuery || require("jquery"), t.propHooks.scrollTop = t.propHooks.scrollLeft = {
            get: function(e, t) {
                var a = null;
                return ("HTML" === e.tagName || "BODY" === e.tagName) && ("scrollLeft" === t ? a = window.scrollX : "scrollTop" === t && (a = window.scrollY)), null == a && (a = e[t]), a
            }
            ...
            configure: function(e) {
                return t.extend(a.config, e || {}), this
            },
            scroll: function(e, s) {
                var r, o, n, c, l, i, u, d, h, p, m, f, C, v, g, $, x, T;
                return r = e.pop(), o = r.$container, n = r.$target, i = o.prop("tagName"), c = t("<span/>").css({
                    position: "absolute",
                    top: "0px",
                    left: "0px"
                }), l = o.css("position"), o.css({
                    position: "relative"
                }), c.appendTo(o), m = c.offset().top, f = n.offset().top, C = f - m - parseInt(s.offsetTop, 10), v = c.offset().left, g = n.offset().left, $ = g - v - parseInt(s.offsetLeft, 10), u = o.prop("scrollTop"), d = o.prop("scrollLeft"), c.remove(), o.css({
                    position: l
                }), x = {}, T = function(t) {
                    return 0 === e.length ? "function" == typeof s.callback && s.callback() : a.scroll(e, s), !0
                }, s.onlyIfOutside && (h = u + o.height(), p = d + o.width(), C > u && h > C && (C = u), $ > d && p > $ && ($ = d)), C !== u && (x.scrollTop = C), $ !== d && (x.scrollLeft = $), o.prop("scrollHeight") === o.width() && delete x.scrollTop, o.prop("scrollWidth") === o.width() && delete x.scrollLeft, null != x.scrollTop || null != x.scrollLeft ? o.animate(x, {
                    duration: s.duration,
                    easing: s.easing,
                    complete: T
                }) : T(), !0
            ...
            fn: function(e) {
                ...
                for (r = t.extend({}, a.config, e), o = c.parent(), n = o.get(0); 1 === o.length && n !== document.body && n !== document;) {
                    var l, i;
                    l = "visible" !== o.css("overflow-y") && n.scrollHeight !== n.clientHeight, i = "visible" !== o.css("overflow-x") && n.scrollWidth !== n.clientWidth, (l || i) && (s.push({
                        $container: o,
                        $target: c
                    }), c = o), o = o.parent(), n = o.get(0)
                }
                return s.push({
                    $container: t("html"),
                    $target: c
                }), "all" === r.durationMode && (r.duration /= s.length), a.scroll(s, r), this
            }
        }, t.ScrollTo = t.ScrollTo || a, t.fn.ScrollTo = t.fn.ScrollTo || a.fn, a
    });
...
$(document).ready(function() {
    $(".rating span").click(function() {
        var e = $(this).parent().data("question-id"),
            ...
            a = $(this).parent().data("rated");
        a || (updateRating(e, t, $(this).parent()), $(this).siblings().removeClass("active animated rubberBand"), $(this).addClass("active animated rubberBand"))
        ...
        if (!t) {
            ...
            var a = $(this).parent().parent().find(".likeAmount");
